...
def main():
    """
    main script function to pull in data and calc fields then saves Spark
    Dataframe as JSON  one to S3
    """
    ...
    def last5_byteam(df=df_total, team='Arsenal'):
        ...
        dfteam['goals'] = dfteam.apply(__goals,
                                       axis=1,
                                       home='FTHG',
                                       away='FTAG')
        dfteam['shots_on'] = dfteam.apply(__goals,
                                          axis=1,
                                          home='HST',
                                          away='AST')
        ...
        dfteam['last5goals'] = dfteam['goals'].rolling(5, min_periods=1) \
                                              .sum() \
                                              .shift(1) \
                                              .fillna(0)

        dfteam['last5shots_on'] = dfteam['shots_on'].rolling(5,
                                                             min_periods=1) \
                                                    .sum() \
                                                    .shift(1) \
                                                    .fillna(0)
    ...
    # sum at end of method chain is dummy sum
    df_total2 = (reduce(lambda df1, df2:
                        df1.append(df2,
                                   ignore_index=True),
                        pds_list)).groupby(['idx_test',
                                            'Date'])['homeLast5goals',
                                                     'awayLast5goals',
                                                     'homeLast5shots_on',
